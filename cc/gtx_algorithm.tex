...
correctness, and show a detailed example of its execution.  Finally, we provide a conjecture on
the number of iterations needed to finish.

\paragraph{\extractStar{}}\label{par:extractstar}%
\begin{wrapfigure}{o}[10mm]{45mm}
  \centering
  \includegraphics[width=.9\linewidth]{assets/tikz/gtx_star_tikz.pdf}
  \caption[A sample star]{\small A sample star created during the \tth{} collapse level. The black node
...
$u$, \ie{} $G[u] \equiv \mathcal{N}(u)$. Finally $membership$ is the map storing for each node the
index of the star it belongs to. This map is updated by the
\textsc{Star} function, which creates a star given a center $c$, a list $periphery$ of
...
times in total. Then we build the corresponding star (line 11--14). In total, we test the membership
of $|V|$
...
$G_{t+1}$ where each star $s_u$ from \extractStar{} becomes a node and where there is a link between two
...
\begin{figure}[bpt]
  \centering
  \includegraphics[width=0.4\linewidth]{assets/tikz/gtx_cross_edges_tikz.pdf}
  \caption[Cross edges between two stars]{In this graph, there are four possible edges between the two
  stars $s_1$ and $s_2$, and all are part of $cross\_edges[(s_1, s_2)]$, here represented in light
red. Those edges are labeled with the sum of eccentricity of their underlying endpoints. White node
have an eccentricity of $0$, and gray node an eccentricity of $1$. The edge with the
minimal eccentricity is linking a peripheral node of $s_1$ directly to the center of $s_2$.}
...
\begin{center}
  ...
  \begin{algorithmic}[1]
    \Function{\collapseStar{}}{$E_t,\,membership,\,eccentricity$}
      ...
      \For{\textbf{every} edge $(u, v)$ in $E_t$}
      ...
      \Let{$s_v$}{$membership[v]$}
        ...
        \EndIf
      \EndFor
      \For{\textbf{every} pair of will-be connected stars $(s_u,s_v)$ in $cross\_edges$}
        \Let{$possible\_underlying\_edges$}{$cross\_edges[(s_u, s_v)]$}
        %\marginpars{\small Line 11 is actually a simplification since the $eccentricity$ array is
        %only defined for nodes in $V_0$ so when \collapseStar{} is called on \eg{} $G_3$, $u$
        %and $v$ are nodes of $V_3$ and we have to retrieve the actual corresponding edge in $E_0$.
        %Maybe I could simply add a map from $E_t$ to $E_0$ like in the real python code…}
...
\noindent\begin{tabulary}{\fullpage}{lllL}
  \toprule
...
this point, we revisit every outer edge to build the spanning tree (line 13).
...
\begin{proof}[Proof of \autoref{prop:gtx_correct}]
  ...
  singleton and \collapseStar{} does not return any outer edges. At this point, \gtx{} finishes,
  proving that the number of iterations $K$ satisfies $K \leq |V_0|$.

 Then we analyze the time complexity. We already know that during the \tth{} iteration,
 ...
 star, although it is not clear at the moment how it would affect the quality of the resulting tree.}
 ...
 also takes $O(K|E_0|)$ time, which is thus the overall complexity of the \gtx{} algorithm.
  ...
  including eventually $G_0$. This process is illustrated in \autoref{fig:gtx_spanning_tree} and
  works as follow: when moving from $G_{t+1}$ to $G_t$, we expand each node of $G_{t+1}$ into a star
...
pessimistic. Indeed, we expect the number of nodes to decrease by more than one at each step,
potentially leading to only $O(\log |V_0|)$ iterations.

\paragraph{Example of \gtx{}}
\label{par:exemple_of_gtx}

We illustrate the operation of the \gtx{} algorithm on a small example.
...
We then call \collapseStar{}. This will connect all possible pairs of stars. For instance, the edge
...
be the center of a star. We again assume they are picked according to their index and therefore we
...
of the spanning tree, that is the test edges, and compute how long are the paths joining their
endpoints. Indeed, this is the second quantity appearing in the mistakes bound of
equation~\eqref{eq:stretch_mistakes}. As shown in \autoref{tab:gtx_example_stretch}, this average
length is $7$.

\begin{table}[htpb]
  \centering
  \caption{Length of the paths not in the resulting spanning tree.}
...
\paragraph{Discussion on the number of iterations needed}\label{par:number_of_iteration}%
...
This will results in $\nicefrac{2n}{5}$ stars organized in a line, which is less than half of $n$.
Thus we can conclude that in this case, \gtx{} will finish after $O(\log n)$ iterations. Note that a barbell graph (two
cliques connected by a line) would requires a number of iterations logarithmic in the length of that
central line, despite having many more nodes and edges.
\begin{figure}[bth]
  \centering
...
In fact, it is helpful to look at this question from the other direction. Namely, we can wonder what
is the minimum number $n(K)$ of nodes necessary for a graph to requires at least $K$ collapses
before being reduced to a single node. Checking by hand, we have that $n(1)=2$, $n(2)=4$ and
$n(3)=8$. Furthermore, this number is achieved by a line or circle consisting of exactly $2^K$
nodes. By induction, we can even show that on any line or circle graphs with $2^n$ nodes, the \gtx{}
algorithm will terminate after $n$ collapses. Assuming we could prove that $n(K)=2^K$, given a graph
$G$ with $n=2^{\log_2 n}$ nodes, by definition of $n(K)=2^K$, it would require less than
$\ceil{\log_2 n} = O(\log n)$ collapses before being reduced to a single node. However, we were not
able to rule out the possibility that there is a graph more parsimonious than a line still able to
survive the same number of collapses. Therefore, we cannot conclude that $n(K)=2^K$ and we leave
this statement as a conjecture.

\iffalse
This suggests unsurprisingly that the diameter of the graph
...
with the same highest degree are broken arbitrarily. This allows for an efficient
implementation, and simplifies the analysis of the resulting sequence of stars and therefore the
induced spanning tree. However, it can be detrimental in an adversarial context, where we could end up with a
tree forcing a lot of mistakes. We add an element of randomization to \extractStar{} by letting it
use two optional arguments, a \emph{threshold function} $\tau$ or a \emph{degree function}
...
to produce spanners by stopping early. Basically if we stop at iteration $t$, we output the graph
$G_t$ (which in general is not a tree) with its edges unfolded to lie in $E_0$. This corresponds to
a trade off between having more edges than $|V_0|-1$ but potentially making shorter connection.
This would also be interesting for the \esp{} problem. Assuming
the treewidth of $G_t$ is decreasing with $t$, we could try to show there is \enquote{reasonable}
