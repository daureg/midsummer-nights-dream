...
\begin{lemma}
  ...
  During the execution of the \gtx{} algorithm, for all $t \leq K-2$, $|V_{t+2}| \leq
  \frac{2}{3}|V_t|$.
\end{lemma}
\begin{marginfigure}
  \centering
...
\begin{proof}
  ...
  are gonna form singleton stars. This means there will be at most $\frac{1-\alpha}{2}n_t$
  ...
  \leq (\frac{1-\alpha}{2} + \alpha)n_t = \frac{1+\alpha}{2}n_t = \frac{19}{20}n_t$ and we get a
  constant rate reduction of the number of nodes. On the other hand, if $\alpha > \lambda$, we could
  be in the situation of the previous example and have $\alpha$ tends to $1$. Then we only have a
  few proper stars and by construction, each singleton star is connected to least one proper star.
  In the worst case, those connections are uniform so that each of these proper star has a degree
  ...
  that in any event, $n_{t+2}\leq \frac{2}{3}n_t$ as claimed.
\end{proof}
